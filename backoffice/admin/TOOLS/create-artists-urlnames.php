...
$sql_query = 'SELECT id, name FROM artists WHERE (urlname="") LIMIT 100';
...
while ($row = mysqli_fetch_array($result)) {
	$newname = create_urlname($row['name'], 1, 1);
	$basename = $newname;

	$inum = 1;

	$check_query = 'SELECT name, urlname FROM artists WHERE urlname="' . mysqli_real_escape_string($sql, $newname) . '"';
	...
	$update_query = 'UPDATE artists SET urlname="' . mysqli_real_escape_string($sql, $newname) . '" WHERE id="' . intval($row['id']) . '"';
